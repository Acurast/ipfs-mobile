...
// The point of a seed: the peer id survives the node it was built for, so a node
// coming back after an idle shutdown is one its peers have already met.
...
// A seed of the wrong length is refused rather than padded or truncated, which
// would hand back a peer id the caller never asked for.
...
// New validates the configuration, so a wrong seed fails there rather than at the
// first download - by which point a caller is waiting on content.
