...
// Pins the age at which a snapshot stops being read.
...
// Pins that repeated writes leave one file, so nothing partial is left to find.
...
// The end to end case: a node that used the DHT records what it met, and records
// only the peers it discovered.
...
// Pins which addresses survive the filter, either side of both rules.
