...
// seeding from one asks less of the network than discovering it again.
...
//
// A reader sees one snapshot or the other, never half of one, since a node killed
// partway through - which Android does to background work - would otherwise leave
// a file the next start has to distrust.
...
// found usable, minus the configured ones, which arrive in the configuration
// anyway.
